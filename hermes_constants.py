...
def _get_platform_default_hermes_home() -> Path:
    ...
    Upstream helper, kept available for callers/tests that want the vanilla
    Hermes location. The fork's own default remains ``~/.hermes_turbo`` via
    :func:`_default_home`.
    """
...
def get_hermes_home() -> Path:
    ...
    Reads HERMES_TURBO_HOME/TOTA_HOME first, then legacy HERMES_HOME, then
    falls back to ~/.hermes_turbo.
