...
def test_local_hermes_turbo_home_version_matches_pyproject():
    version_file = REPO_ROOT / ".hermes-turbo" / "version"
    pyproject = (REPO_ROOT / "pyproject.toml").read_text()
...
def test_identity_customization_doc_exists():
    doc = REPO_ROOT / "docs" / "hermes-turbo-identity-customization.md"
    assert doc.exists()
    text = doc.read_text()
