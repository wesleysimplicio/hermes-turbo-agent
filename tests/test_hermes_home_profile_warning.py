...
When home env vars are unset but an active_profile file indicates a non-default
profile is active, get_hermes_home() should:
  1. STILL return ~/.hermes_turbo (raising would brick 30+ module-level callers)
...
class TestGetHermesHomeProfileWarning:
    def test_classic_mode_no_active_profile_no_warning(
        ...
        """Classic mode: no active_profile file -> silent, returns ~/.hermes_turbo."""
        result = fresh_constants.get_hermes_home()
        assert result == tmp_path / ".hermes_turbo"
        assert "HERMES_HOME fallback" not in capsys.readouterr().err

    def test_default_active_profile_no_warning(
        self, fresh_constants, tmp_path, capsys
    ):
        """active_profile=default -> still no warning, returns ~/.hermes_turbo."""
        hermes_dir = tmp_path / ".hermes_turbo"
        hermes_dir.mkdir()
    ...
    def test_named_profile_unset_home_warns_once(
        ...
        """active_profile=coder + home env unset -> warn loudly, still return fallback."""
        hermes_dir = tmp_path / ".hermes_turbo"
        hermes_dir.mkdir()
    ...
    def test_hermes_home_set_suppresses_warning(
        ...
        profile_dir = tmp_path / ".hermes_turbo" / "profiles" / "coder"
        profile_dir.mkdir(parents=True)
        (tmp_path / ".hermes_turbo" / "active_profile").write_text("coder\n")
