...
```bash
python3 scripts/hermes_turbo_daily_update.py \
  --repo /Users/wesleysimplicio/Projetos/contribuicoes/hermes/hermes-turbo-agent \
  --python-version 3.14.5
```

The routine:

1. Creates an isolated checkout under `~/.local/state/hermes-turbo-agent/hermes-sync/`.
...
6. Verifies Hermes Turbo-specific markers such as `HERMES_TURBO_HOME`, `orjson`, `msgspec`,
...
12. Commits and pushes a dated `codex/hermes-turbo-daily-*` branch when all
   checks pass.
...
```bash
~/.local/state/hermes-turbo-agent/hermes-sync/latest.md
```
...
```bash
python3 scripts/hermes_turbo_daily_update.py --repo . --python-version 3.14.5
```

Dry-run without commit/push:

```bash
python3 scripts/hermes_turbo_daily_update.py --repo . --python-version 3.14.5 --dry-run
```

Uninstall the daily job:

```bash
python3 scripts/install_hermes_turbo_daily_update_launchd.py --uninstall
```
