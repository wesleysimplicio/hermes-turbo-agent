...
- ⏳ #37 — Phase 1.5 `hermes_state.py` `orjson` migration + SQLite round-trip audit. Highest-risk; behind `HERMES_TURBO_FAST_STATE=1` feature flag.
...
- The `.hermes-turbo/` repo directory is the source of truth for runtime defaults; `$HERMES_TURBO_HOME` is the operator-mutable runtime home.
