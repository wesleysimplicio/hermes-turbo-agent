...
from hermes_cli.dashboard_auth.public_paths import (
    PUBLIC_API_PATHS as _UPSTREAM_PUBLIC_API_PATHS,
)

# Fork (turbo) extras layered on top of the upstream allowlist. These are
# read-only, localhost-bound endpoints the fork exposes that upstream lacks:
# the plugins rescan trigger and the performance dashboard (issue #137), which
# reads only ~/.hermes/telemetry/*.jsonl.
_PUBLIC_API_PATHS: frozenset = _UPSTREAM_PUBLIC_API_PATHS | frozenset({
