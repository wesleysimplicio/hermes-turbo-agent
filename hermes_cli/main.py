...
_BUILTIN_SUBCOMMANDS = frozenset(
    {
        ...
        "migrate", "migrate-from-openclaw", "moa",
...
def main():
    ...
    # Fork-specific update flags: re-grafted onto upstream's extracted
    # ``build_update_parser`` (which has no --check-main/--apply). These drive
    # the fork's ``run_check_main`` fast-forward-against-origin path.
    _update_parser = subparsers.choices["update"]
